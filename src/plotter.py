...
def plot_std_on_best_x(data, name, stopped_in_iteration):
    fig, ax = plt.subplots()
    ...
    if stopped_in_iteration != -1:
        ax.axvline(stopped_in_iteration, ls="--", color="r", label="stop")
    ax.legend()
    plt.savefig(plots_path/(f"{name}_"+f"std_x0_vs_best_x0.jpg"))
    plt.close(fig)

def plot_mean_on_best_x(data, name, stopped_in_iteration):
    fig, ax = plt.subplots()
...
def plot_best_individual_value_vs_std_x(data, name, stopped_in_iteration):
    fig, ax = plt.subplots()
    ...
    lines = line1 + line2
    if stopped_in_iteration != -1:
        vertical = ax.axvline(stopped_in_iteration, ls="--", color="r", label="stop")
        lines += [vertical]
...
def plot_best_individual_value_vs_mean_x(data, name, stopped_in_iteration):
    fig, ax = plt.subplots()
